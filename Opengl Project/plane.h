...
public:
    ...
    float I[3] = {0};
    coordinate3f color;
    coordinate3f ref, centroid;
    coordinate3f normal, vertexNormal[3];
    ...
    void vertexIntensities()
    {
        for (int i = 0; i < 3; i++)
        {
            I[i] = Ia*Ka + Ip * kd * (!normal^ !(vertex[i]- worldprops::pointlight));
            I[i] = I[i] > 1 ? 1 : I[i];
        }
    }
    ...
    void newdraw()
    {
        worldprops::pointlight = worldprops::camera;
        vertexIntensities();
